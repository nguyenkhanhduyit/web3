...
async function main() {
  ...
  // Add liquidity for each pair with exact ratio
  for (let i = 0; i < tokenPairs.length; i++) {
    ...
    // Calculate the exact ratio from current reserves
    // For existing pools, we need to maintain the same ratio
    const ratio = reserves[1].mul(ethers.utils.parseUnits("1", token1Info.decimals)).div(reserves[0]);
    console.log(`Tỷ lệ hiện tại ${token2Info.symbol}/${token1Info.symbol}: ${ethers.utils.formatUnits(ratio, token2Info.decimals)}`);

    // Add a small amount (10k tokens) with the exact same ratio
    const additionalAmount = 10000; // 10k tokens
    const amount0 = ethers.utils.parseUnits(additionalAmount.toString(), token1Info.decimals);
    
    // Calculate amount1 based on the exact ratio
    const amount1 = amount0.mul(ratio).div(ethers.utils.parseUnits("1", token1Info.decimals));

    console.log(`\nSố lượng thanh khoản bổ sung (tỷ lệ chính xác):`);
  ...
  fs.writeFileSync(
    path.resolve(infoDir, "ExactRatioLiquidity.json"),
  ...
  console.log(`Thông tin chi tiết đã lưu vào: info/ExactRatioLiquidity.json`);
