...
async function main() {
  console.log("Đang thêm thanh khoản bổ sung cân bằng đến SimpleDEX...\n");
  ...
  console.log(`Sẽ thêm thanh khoản bổ sung cho ${tokenPairs.length} cặp token pools:`);
  ...
  // Add additional liquidity for each pair
  for (let i = 0; i < tokenPairs.length; i++) {
    ...
    console.log(`\n${"=".repeat(60)}`);
    console.log(`Đang thêm thanh khoản bổ sung cho pool ${i + 1}/${tokenPairs.length}: ${token1Name}-${token2Name}...`);
    console.log(`Token1 (${token1Info.symbol}): ${token1Info.tokenAddress}`);
    console.log(`Token2 (${token2Info.symbol}): ${token2Info.tokenAddress}`);

    // Kiểm tra trạng thái hiện tại của pool
    ...
    // Mục tiêu: Tạo ra thêm khoảng 10 LP tokens cho mỗi pool
    ...
    // Điều chỉnh số lượng dựa trên decimals để cân bằng
    let amount0BN, amount1BN;
    
    if (token1Info.decimals === maxDecimals) {
      // Token1 có decimals cao hơn, cần thêm token2 để cân bằng
      amount0BN = ethers.utils.parseUnits(baseAmount.toString(), token1Info.decimals);
      amount1BN = ethers.utils.parseUnits((baseAmount * Math.pow(10, decimalDiff)).toString(), token2Info.decimals);
    } else {
      // Token2 có decimals cao hơn, cần thêm token1 để cân bằng
      amount0BN = ethers.utils.parseUnits((baseAmount * Math.pow(10, decimalDiff)).toString(), token1Info.decimals);
      amount1BN = ethers.utils.parseUnits(baseAmount.toString(), token2Info.decimals);
    }

    console.log(`\nSố lượng thanh khoản bổ sung cân bằng:`);
    ...
    // Ước tính LP tokens sẽ nhận được (đơn giản hóa)
    console.log(`Ước tính LP tokens bổ sung: ~10 LP tokens`);

    try {
      // Add additional liquidity
      console.log("\nĐang thêm thanh khoản bổ sung...");
      ...
      console.log("Thanh khoản bổ sung đã được thêm thành công!");
      ...
      console.log(`\nThông tin pool sau khi thêm thanh khoản bổ sung:`);
      ...
      const poolInfo = {
        ...
        previousReserves: {
          reserve0: ethers.utils.formatUnits(existingReserves[0], token0Info.decimals),
          reserve1: ethers.utils.formatUnits(existingReserves[1], token1InfoCanonical.decimals)
        },
        newReserves: {
      ...
      console.log("Thất bại khi thêm thanh khoản bổ sung:", error.message);
  ...
  fs.writeFileSync(
    path.resolve(infoDir, "AdditionalBalancedLiquidity.json"),
    JSON.stringify(summaryInfo, null, 2)
  );

  console.log(`\n${"=".repeat(60)}`);
  console.log("HOÀN THÀNH THÊM THANH KHOẢN BỔ SUNG CÂN BẰNG CHO TẤT CẢ CÁC CẶP TOKEN!");
  console.log(`${"=".repeat(60)}`);
  console.log(`Tổng kết :`);
  console.log(`Tổng số cặp token: ${tokenPairs.length}`);
  console.log(`Thành công: ${successCount}`);
  console.log(`Thất bại: ${failCount}`);
  console.log(`Thông tin chi tiết đã lưu vào: info/AdditionalBalancedLiquidity.json`);
