...
/**
 * Script test swap token từ token1 sang token2
 * Chức năng:
 ...
 */
async function main() {
  console.log("🔄 Đang test swap từ token1 sang token2...\n");
  ...
  const testResults: any = {
    ...
    testType: "swap_token1_to_token2",   // Loại test
  ...
  // Định nghĩa số lượng token1 sẽ swap
  ...
  try {
    ...
    console.log("\n🧮 Đang ước tính số lượng token2 sẽ nhận được...");
    const estimatedAmountOut = await simpleDex.getAmountOut(swapAmount, token1Info.tokenAddress, token2Info.tokenAddress);
    console.log(`📊 Ước tính sẽ nhận: ${ethers.utils.formatUnits(estimatedAmountOut, token2Info.decimals)} ${token2Info.symbol}`);

    // Bước 4: Phê duyệt token1 để SimpleDEX có thể swap
    console.log("\n🔐 Đang phê duyệt token1 để swap...");
    ...
    const swapTx = await simpleDex.swapExactTokensForTokens(
      ...
      estimatedAmountOut.mul(95).div(100), // Số lượng token2 tối thiểu muốn nhận (95% của ước tính)
      token1Info.tokenAddress,       // Địa chỉ token1
      token2Info.tokenAddress,       // Địa chỉ token2
      deployer.address,              // Địa chỉ người nhận token2
    ...
    const balance1Change = balance1Before.sub(balance1After); // Số token1 đã mất
    const balance2Change = balance2After.sub(balance2Before); // Số token2 đã nhận

    console.log(`\n📊 Thay đổi số dư:`);
    console.log(`   • ${token1Info.symbol} đã mất: ${ethers.utils.formatUnits(balance1Change, token1Info.decimals)}`);
    ...
    const actualRate = balance2Change.mul(ethers.utils.parseUnits("1", token1Info.decimals)).div(balance1Change);
    console.log(`💱 Tỷ giá swap thực tế: 1 ${token1Info.symbol} = ${ethers.utils.formatUnits(actualRate, token2Info.decimals)} ${token2Info.symbol}`);
    ...
    testResults.testResults.swapToken1ToToken2 = {
      ...
      swapDetails: {
        inputAmount: ethers.utils.formatUnits(swapAmount, token1Info.decimals), // Số lượng token1 đã swap
        estimatedOutput: ethers.utils.formatUnits(estimatedAmountOut, token2Info.decimals), // Số lượng token2 ước tính
        actualOutput: ethers.utils.formatUnits(balance2Change, token2Info.decimals), // Số lượng token2 thực tế nhận
        actualRate: ethers.utils.formatUnits(actualRate, token2Info.decimals) // Tỷ giá thực tế
      },
      reservesBefore: {
      ...
      balanceChanges: {
        token1Lost: ethers.utils.formatUnits(balance1Change, token1Info.decimals), // Số token1 đã mất
    ...
    testResults.testResults.swapToken1ToToken2 = {
      ...
      swapAmount: ethers.utils.formatUnits(swapAmount, token1Info.decimals), // Số lượng token1 đã thử swap
      testTokens: {
        token1: {
          name: token1Name,
          symbol: token1Info.symbol,
  ...
  fs.writeFileSync(
    path.resolve(infoDir, "SwapToken1ToToken2Test.json"),
    JSON.stringify(testResults, null, 2)
  );

  // ===== HIỂN THỊ TỔNG KẾT =====
  console.log("\n" + "=".repeat(50));
  console.log("✅ HOÀN THÀNH TEST SWAP TOKEN1 → TOKEN2!");
  console.log("=".repeat(50));
  
  if (testResults.testResults.swapToken1ToToken2.status === "passed") {
  ...
  console.log("📁 Kết quả đã lưu vào: info/SwapToken1ToToken2Test.json");
  
  console.log("\n🚀 BƯỚC TIẾP THEO:");
  console.log("-".repeat(40));
  console.log("1. Chạy 05d-test-swap-token2-to-token1.ts để test swap ngược");
  console.log("2. Chạy 05e-test-remove-liquidity.ts để test rút thanh khoản");
  console.log("3. Hoặc chạy 05f-test-all-dex-features.ts để test tất cả");
}
