...
async function main() {
  console.log("Thêm thanh khoản cuối cùng với số lượng cân bằng...\n");
  ...
  console.log(`Sẽ thêm thanh khoản cuối cùng cho ${tokenPairs.length} cặp token pools:`);
  ...
  // Add final balanced liquidity for each pair
  for (let i = 0; i < tokenPairs.length; i++) {
    ...
    console.log(`\n${"=".repeat(60)}`);
    console.log(`Đang thêm thanh khoản cuối cùng cho pool ${i + 1}/${tokenPairs.length}: ${token1Name}-${token2Name}...`);
    ...
    // Calculate balanced amounts to achieve ~10 LP tokens
    // Target: sqrt(amount0 * amount1) ≈ 10 * 1e18
    // For 1:1 ratio pools, we need: sqrt(amount^2) = amount ≈ 10 * 1e18
    
    // Calculate the base amount that will give us ~10 LP tokens
    // For tokens with different decimals, we need to adjust
    const targetLiquidity = ethers.utils.parseUnits("10", 18); // 10 LP tokens
    
    // Calculate amounts based on decimals difference
    ...
    let baseAmount0, baseAmount1;
    
    if (token1Info.decimals === maxDecimals) {
      // Token1 has higher decimals, need more token2 to balance
      baseAmount0 = 1000000; // 1M tokens
      baseAmount1 = 1000000 * Math.pow(10, decimalDiff); // Adjust for decimal difference
    } else {
      // Token2 has higher decimals, need more token1 to balance
      baseAmount0 = 1000000 * Math.pow(10, decimalDiff); // Adjust for decimal difference
      baseAmount1 = 1000000; // 1M tokens
    }
    
    // Convert to BigNumber
    const amount0 = ethers.utils.parseUnits(baseAmount0.toString(), token1Info.decimals);
    const amount1 = ethers.utils.parseUnits(baseAmount1.toString(), token2Info.decimals);

    console.log(`\nSố lượng thanh khoản cuối cùng (cân bằng decimals):`);
    ...
    console.log(`Mục tiêu LP tokens: ~10`);

    try {
      // Add final liquidity
      console.log("\nĐang thêm thanh khoản cuối cùng...");
      ...
      console.log("Thanh khoản cuối cùng đã được thêm thành công!");
      ...
      console.log(`\nThông tin pool sau khi thêm thanh khoản cuối cùng:`);
      ...
      console.log("Thất bại khi thêm thanh khoản cuối cùng:", error.message);
  ...
  fs.writeFileSync(
    path.resolve(infoDir, "FinalBalancedLiquidity.json"),
    JSON.stringify(summaryInfo, null, 2)
  );

  console.log(`\n${"=".repeat(60)}`);
  console.log("HOÀN THÀNH THÊM THANH KHOẢN CUỐI CÙNG!");
  console.log(`${"=".repeat(60)}`);
  console.log(`Tổng kết :`);
  console.log(`Tổng số cặp token: ${tokenPairs.length}`);
  console.log(`Thành công: ${successCount}`);
  console.log(`Thất bại: ${failCount}`);
  console.log(`Thông tin chi tiết đã lưu vào: info/FinalBalancedLiquidity.json`);
