...
async function main() {
    ...
    for (const [tokenName, tokenData] of Object.entries(tokenInfo)) {
        ...
        console.log(`Deployer ${tokenName} 
            balance: ${ethers.utils.formatUnits(deployerBalance, tokenData.decimals)} ${tokenData.symbol}`);
        ...
        if (deployerBalance >= deployAmount) {
            ...
            console.log(`Đã chuyển 
                ${ethers.utils.formatUnits(deployAmount, tokenData.decimals)} ${tokenData.symbol} vào faucet`);
