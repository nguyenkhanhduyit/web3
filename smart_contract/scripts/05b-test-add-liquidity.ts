...
async function main() {
  console.log("Test thêm thanh khoản vào SimpleDEX cho tất cả các cặp token...\n");
  ...
  console.log(`Sẽ test thêm thanh khoản cho ${tokenPairs.length} cặp token:`);
  ...
  const testResults: any = {
    ...
    testName: "Test thêm thanh khoản cho tất cả cặp token",
    status: "completed",
  ...
  console.log("\n" + "=".repeat(60));
  console.log("TEST THÊM THANH KHOẢN CHO TẤT CẢ CẶP TOKEN");
  console.log("=".repeat(60));

  // Test thêm thanh khoản cho từng cặp token
  for (let i = 0; i < tokenPairs.length; i++) {
    ...
    console.log(`\n${"=".repeat(60)}`);
    console.log(`Đang test cặp token ${i + 1}/${tokenPairs.length}: ${token1Name}-${token2Name}`);
    console.log(`Token1 (${token1Info.symbol}): ${token1Info.tokenAddress}`);
    console.log(`Token2 (${token2Info.symbol}): ${token2Info.tokenAddress}`);

    const pairResult: any = {
      pairName: `${token1Name}-${token2Name}`,
      token1: { name: token1Name, symbol: token1Info.symbol, address: token1Info.tokenAddress },
      token2: { name: token2Name, symbol: token2Info.symbol, address: token2Info.tokenAddress },
      status: "pending"
    };
    ...
    try {
      ...
      // Lưu kết quả cho cặp này
      pairResult.status = "success";
      pairResult.transactionHash = addLiquidityTx.hash;
      pairResult.gasUsed = receipt.gasUsed.toString();
      pairResult.data = {
        before: {
          reserves: {
      ...
      console.log(`Test thêm thanh khoản cho ${token1Name}-${token2Name} hoàn thành thành công!`);

    } catch (error: any) {
      console.log(`Lỗi khi test thêm thanh khoản cho ${token1Name}-${token2Name}:`, error.message);
      pairResult.status = "failed";
      pairResult.error = error.message;
      testResults.failCount++;
    }

    testResults.pairs.push(pairResult);
  }
  ...
  console.log(`\n${"=".repeat(60)}`);
  console.log("HOÀN THÀNH TEST THÊM THANH KHOẢN CHO TẤT CẢ CẶP TOKEN!");
  console.log(`${"=".repeat(60)}`);
  console.log(`Tổng kết:`);
  console.log(`Tổng số cặp token: ${testResults.totalPairs}`);
  console.log(`Thành công: ${testResults.successCount}`);
  console.log(`Thất bại: ${testResults.failCount}`);
