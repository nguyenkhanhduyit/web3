...
<>
  <nav className='w-full flex md:justify-center justify-between items-center p-4'>
    ...
    <ul className={`md:flex hidden list-none flex-row justify-between items-center flex-initial 
      ${theme === 'dark-mode'?'text-white font-bold':'text-indigo-700 font-bold'}`}>
      {
        menu.map((item, index) => (
          <NavBarItem
    ...
    <div className='flex relative'>
      ...
      toggleMenu ? (
        ...
        className='text-white md:hidden cursor-pointer' onClick={() => setToggleMenu(false)} />
      ) : (
        <HiMenuAlt4 fontSize={28} className={`${theme==='dark-mode'?'text-white':'text-indigo-700'}
      ...
      toggleMenu && (
        <ul className={`z-10 fixed top-0 -right-2 p-3 w-[70vw] h-screen shadow-2xl md:hidden list-none
          flex flex-col justify-start items-end rounded-md blue-glassmorphism  animate-slide-in
           ${theme === 'dark-mode'?'text-white':'text-indigo-700'}`}>
      ...
      {
      showModalMarket && (<ModalMarket theme={theme} onClose={() => setShowModalMarket(false)} />)
      }
      {
      showModalTransaction && (<ModalTransaction theme={theme} onClose={() => setShowModalTransaction(false)}/>)
      }
</>
  );
};
